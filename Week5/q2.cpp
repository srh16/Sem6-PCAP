...
int main (void) {
	...
	string *A = (string *)malloc(sizeof(string) * LIST_SIZE);
	int k = 0;
	...
	cl_mem mem_obj_a = clCreateBuffer(context, CL_MEM_READ_ONLY, LIST_SIZE * sizeof(string), NULL, &ret);

	cl_mem mem_obj_b = clCreateBuffer(context, CL_MEM_WRITE_ONLY, LIST_SIZE * sizeof(string), NULL, &ret);


	// 6. Copy the list A to the respective memory buffers

	ret = clEnqueueWriteBuffer(command_queue, mem_obj_a, CL_TRUE, 0, LIST_SIZE * sizeof(string), A, 0, NULL, NULL);
	...
	cl_kernel kernel = clCreateKernel(program, "one_comp", &ret);
	...
	string *B = (string *)malloc(sizeof(string) * LIST_SIZE);

	ret = clEnqueueReadBuffer(command_queue, mem_obj_b, CL_TRUE, 0, LIST_SIZE * sizeof(string), B, 0, NULL, NULL);
