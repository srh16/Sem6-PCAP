...
int main (void) {
	...
	cl_mem mem_obj_b = clCreateBuffer(context, CL_MEM_WRITE_ONLY, len *N* sizeof(char), NULL, &ret);
	...
	cl_kernel kernel = clCreateKernel(program, "dup", &ret);
	...
	char *B = (char *)malloc(sizeof(char) * len*N);

	ret = clEnqueueReadBuffer(command_queue, mem_obj_b, CL_TRUE, 0, len *N* sizeof(char), B, 0, NULL, NULL);
